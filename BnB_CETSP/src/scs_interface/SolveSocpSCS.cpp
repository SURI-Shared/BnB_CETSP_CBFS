...
SolveSocpSCS::SolveSocpSCS(Data * instance, vector<int>& in_sequence):sequence(in_sequence),objectData(instance),sizeProblem(in_sequence.size()),f_value(0),solution(),workspace_ptr(),settings(),as_tridiagonal(false),info(){
...
SolveSocpSCS::SolveSocpSCS(Data * instance, int size_seq):sequence(),objectData(instance),sizeProblem(size_seq),f_value(0),solution(),workspace_ptr(),settings(),as_tridiagonal(false),info(){
    scs_set_default_settings(&settings);
    settings.verbose=false;
}

SolveSocpSCS::SolveSocpSCS(Data * instance, vector<int>& in_sequence,bool use_tridiagonal):sequence(in_sequence),objectData(instance),sizeProblem(in_sequence.size()),f_value(0),solution(),workspace_ptr(),settings(),as_tridiagonal(use_tridiagonal),info(){
    scs_set_default_settings(&settings);
    settings.verbose=false;
    //TODO make sure SCS tolerances are comparable to Clarabel
    createModel(sequence);
...
SolveSocpSCS::SolveSocpSCS(Data * instance, int size_seq, bool use_tridiagonal):sequence(),objectData(instance),sizeProblem(size_seq),f_value(0),solution(),workspace_ptr(),settings(),as_tridiagonal(use_tridiagonal),info(){
    scs_set_default_settings(&settings);
    settings.verbose=false;
}
...
void SolveSocpSCS::createModel(vector<int>& sequence){
    if(as_tridiagonal){
        createTridiagonalModel(sequence);
    }else{
        createRegularModel(sequence);
    }
}
/*
void SolveSocpSCS::createRegularModel(vector<int>& sequence)
...
/*
void SolveSocpSCS::createTridiagonalModel(vector<int>& sequence)

store a ScsWork in this->workspace_ptr and populate its data for the SOCP to visit neighbhoods in the order defined by sequence, with column order chosen such that A'A is tridiagonal

Parameters: sequence : vector<int>&
                the order to visit neighborhoods in. The first element must be the depot, of radius 0. The path returns to the depot after visiting sequence[-1]
*/
void SolveSocpSCS::createTridiagonalModel(vector<int>& sequence){
    //decision vector is [f0 f2...fm x10 x20...xm0...xmndim]
    size_t m=sequence.size()-1;
    size_t nf=m+1;
    size_t nx=SOCP_NDIM*m;
    nvar=nf+nx;
    size_t xstart=nf;
    size_t ystart=xstart+m;
    size_t zstart=ystart+m;
    sizeProblem=nf;
    this->sequence=sequence;
    //Quadratic cost matrix is empty
    ...
    for(size_t i=0;i<m;i++){
        ...
        row+=1;//row of zeros
        //x
        tripletList.push_back(Triplet(row,xstart+i,1));
        b[row]=objectData->getCoordx(sequence[i+1]);
        row+=1;
        //y
        tripletList.push_back(Triplet(row,ystart+i,1));
        b[row]=objectData->getCoordy(sequence[i+1]);
        row+=1;
        //z
        tripletList.push_back(Triplet(row,zstart+i,1));
        b[row]=objectData->getCoordz(sequence[i+1]);
        row+=1;
    }
    ...
    for(size_t i=0;i<nf;i++){
        ...
        if(i==0){
            ...
            row+=1;
            //y
            tripletList.push_back(Triplet(row,ystart,-1));//x11
            b[row]=-objectData->getCoordy(sequence[0]);
            row+=1;
            //z
            tripletList.push_back(Triplet(row,zstart,-1));//x12
            b[row]=-objectData->getCoordz(sequence[0]);
            row+=1;
            ...
            //s=[fi,depot-xm]
            //x
            tripletList.push_back(Triplet(row,xstart+m-1,1));//xm0
            b[row]=objectData->getCoordx(sequence[0]);
            row+=1;
            //y
            tripletList.push_back(Triplet(row,ystart+m-1,1));//xm1
            b[row]=objectData->getCoordy(sequence[0]);
            row+=1;
            //z
            tripletList.push_back(Triplet(row,zstart+m-1,1));//xm2
            b[row]=objectData->getCoordz(sequence[0]);
            row+=1;
        }else{
            //||x(i+1)j-xij||<=fi
            //s=[fi,x(i+1)-xi]
            for(size_t j=0;j<SOCP_NDIM;j++){
                tripletList.push_back(Triplet(row,xstart+j*m+i-1,1));//xij
                tripletList.push_back(Triplet(row,xstart+j*m+i,-1));//x(i+1)j
                b[row]=0;
                row+=1;
            }
        }
...
double SolveSocpSCS::getSolutionX( int idx){
    ...
    if(!as_tridiagonal){
        ...
        return solution.x[sizeProblem+idx-1];
    }
}
double SolveSocpSCS::getSolutionY( int idx){
    ...
    if(!as_tridiagonal){
        ...
        return solution.x[sizeProblem+sizeProblem-1+idx-1];
    }
}
double SolveSocpSCS::getSolutionZ( int idx){
    ...
    if(!as_tridiagonal){
        ...
        return solution.x[sizeProblem+sizeProblem-1+sizeProblem-1+idx-1];
    }
}
