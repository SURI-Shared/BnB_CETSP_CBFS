...
int main(int argc, char** argv) 
{
   ...
   SCSWarmStartHandler warm_start_handler(true);
   //solve model
   ...
   nodes_for_warmstart.emplace(root->id,BnBNodeForWarmStart(root->pts,solveSocpPtr->primals(),solveSocpPtr->duals(),solveSocpPtr->slacks(),true));
            ...
            for (int span_id = 0; span_id < current->insert_spans.size(); span_id++)
            {
               ...
               for (int pos = start_pos; pos < end_pos; pos++)
               {
                  ...
                  if (!has_lah_node_hit)
                  {
                     ...
                     {
                        tempSbStr.sum += child->lb;
                        nodes_for_warmstart.emplace(child->id,BnBNodeForWarmStart(child->pts,solveSocpPtr->primals(),solveSocpPtr->duals(),solveSocpPtr->slacks(),true));
                     }
                     ...
                     for ( int i2 = 0; i2 < child->pts.size(); i2++ ){
                        tempX[i2] = solveSocpPtr-> getSolutionX(i2);
                        tempY[i2] = solveSocpPtr-> getSolutionY(i2);
                        tempZ[i2] = solveSocpPtr-> getSolutionZ(i2);
                     }
                  ...
                  else
                  {
                     ...
                     if (use_reverse_prune && !has_lah_node_hit)
                     {
                        ...
                        else
                        {
                           ...
                           vector<double> line1_from = {solveSocpPtr->getSolutionX(begin_of_seg_pos), 
                                                         solveSocpPtr->getSolutionY(begin_of_seg_pos), 
                                                         solveSocpPtr->getSolutionZ(begin_of_seg_pos)};
                           vector<double> line2_to = {solveSocpPtr->getSolutionX(end_of_seg_pos), 
                                                      solveSocpPtr->getSolutionY(end_of_seg_pos), 
                                                      solveSocpPtr->getSolutionZ(end_of_seg_pos)};
                           if (!rp_prev_node_checked)
                           {
                              rp_temp_node = child;
                           ...
                           else
                           {
                              ...
                              if(end_of_seg_pos != 0)
                              {
                                 rp_temp_node = child;
                                 rp_temp_node_lb = curr_child_socp_lb;
                                 rp_prev_line1_from = {solveSocpPtr->getSolutionX(begin_of_seg_pos+1), 
                                                         solveSocpPtr->getSolutionY(begin_of_seg_pos+1), 
                                                         solveSocpPtr->getSolutionZ(begin_of_seg_pos+1)};
                                 end_of_seg_pos = (pos == end_pos-2) ? 0 : pos+3;
                                 rp_prev_line2_to = {solveSocpPtr->getSolutionX(end_of_seg_pos), 
                                                      solveSocpPtr->getSolutionY(end_of_seg_pos), 
                                                      solveSocpPtr->getSolutionZ(end_of_seg_pos)};
                              }
                           }
                        ...
                        if (!current->reversible_seg_infos.empty())
                        {
                           ...
                           for (int pos_index = 0; pos_index < child->pts.size()-2; pos_index++)
                           {
                              ...
                              if (current->reversible_seg_infos.count(temp_key) > 0 && !current->reversible_seg_infos[temp_key].empty())
                              {
                                 ...
                                 vector<double> curr_line1_from = {solveSocpPtr->getSolutionX(begin_of_seg_pos), 
                                                                   solveSocpPtr->getSolutionY(begin_of_seg_pos), 
                                                                   solveSocpPtr->getSolutionZ(begin_of_seg_pos)};
                                 vector<double> curr_line2_to = {solveSocpPtr->getSolutionX(end_of_seg_pos), 
                                                                 solveSocpPtr->getSolutionY(end_of_seg_pos), 
                                                                 solveSocpPtr->getSolutionZ(end_of_seg_pos)};
